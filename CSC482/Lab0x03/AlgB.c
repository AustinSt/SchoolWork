...
int main (int argc, int argv) 
{
    ...
    for ( n=1; n<N_max; n=2*n )
    {
        ...
        //expected = (log(n)*log(n))/(log(n/2)*log(n/2));
        //expected = (n/2*log(n))/(((n/2)/2)*log(n/2));
        double hNum = n/2;
        //expected = (log(n)+log(n-2))/(log(n/2)+log((n/2)-2));
        expected = (pow(2, log(n))*log(n))/(pow(2,log(n/2))*log(n/2));
