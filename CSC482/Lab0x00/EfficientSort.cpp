...
int part(int randomArray[], int low, int sizeArray)
{
    int lastPart = randomArray[sizeArray];
    int firstLowChange = (low - 1);

    for(int secondLowChange = low; secondLowChange <= sizeArray-1; secondLowChange++)
    {
        if(randomArray[secondLowChange]<lastPart)
        {
            firstLowChange++;
            swap(&randomArray[firstLowChange], &randomArray[secondLowChange]);
        }
    }
    swap(&randomArray[firstLowChange+1], &randomArray[sizeArray]);
    return (firstLowChange+1);    

}

void quickSort(int randomArray[], int low, int sizeArray)
{
    if(low < sizeArray)
    {
        int halve = part(randomArray, low, sizeArray);

        quickSort(randomArray,low, halve-1);
        quickSort(randomArray, halve+1, sizeArray);
    }
}
...
int main()
{
    ...
    do
    {
        ...
        quickSort(dummyArray, 0, dummysizeArray-1);
    ...
    do
    {
        ...
        quickSort(randomArray, 0, sizeArray-1);
